...
async def doc_upload(request: Request,file: UploadFile = File(...), user_email: str = Form(...), db: Session = Depends(get_db)):
    file_path = await upload_file_to_s3(file, user_email)

    validated = TaskStatusCreate(file_name=file.filename, user_email=user_email, file_path=file_path)
    ...
    return {"filename": file.filename, "Status": 'Processing', "task_id": db_task.task_id}
